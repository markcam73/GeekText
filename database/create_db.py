...
with con:
    ...
    cur.execute("CREATE TABLE Users(UserID INT PRIMARY KEY AUTOINCREMENT, FirstName TEXT, LastName TEXT, HomeAddress TEXT, Email TEXT, username TEXT, password TEXT)")
    cur.execute("DROP TABLE IF EXISTS PaymentInformation")
    cur.execute("CREATE TABLE PaymentInformation(PaymentID INT PRIMARY KEY AUTOINCREMENT, UserID INT, CreditCardNumber TEXT, CreditCardCompany TEXT, ExpirationDate TEXT, SecruityCode TEXT)")
    cur.execute("DROP TABLE IF EXISTS ShippingAddresses")
    cur.execute("CREATE TABLE ShippingAddresses(addressid INT PRIMARY KEY AUTOINCREMENT, userid INT, Street TEXT, City TEXT, State TEXT, Zipcode INT)")
